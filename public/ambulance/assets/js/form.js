...
const extend = VeeValidate.extend;
...
extend("email", {
  ...
  message: "The {_field_} must be a valid",
  computesRequired: true,
});
...
const global = {
  ...
  actions: {
    submitForm({ commit }, params) {
      return axios.post(`/travelport-submit.php`, {
        ...params,
      });
    },
  },
...
const mixins = {
  form: {
    ...
    methods: {
      ...
      onRemoveTravler: function (index) {
        this.travlers = this.travlers.filter((el, i) => {
          return i != index;
        });
      },
      onSubmit: function () {
        ...
        store
          .dispatch("submitForm", {
            travlers: this.travlers,
            contact: this.contact,
          })
          .then((res) => {
            this.payWithPaystack(100);
          });
      },
      payWithPaystack: function (finalPrice = 100) {
      ...
      initToday: function () {
        let now = new Date();
        this.travlers.forEach((element) => {
          element.day = now.getDay();
          element.month = now.getMonth() + 1;
...
function init() {
  ...
  const routes = [{ path: "", component: window["form"] }];
